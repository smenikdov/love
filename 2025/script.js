...
const months = [
    { name: 'january', label: 'Январь' },
    { name: 'february', label: 'Февраль' },
    { name: 'march', label: 'Март' },
    { name: 'april', label: 'Апрель' },
    { name: 'may', label: 'Май' },
    { name: 'june', label: 'Июнь' },
    { name: 'july', label: 'Июль' },
    { name: 'august', label: 'Август' },
    { name: 'september', label: 'Сентябрь' },
    { name: 'october', label: 'Октябрь' },
    { name: 'november', label: 'Ноябрь' },
    { name: 'december', label: 'Декабрь' },
];
...
function init() {
    ...
    for (let i = 0; i < months.length; i++) {
        ...
        const button = $('#button-' + month.name);
        month.button = button;
        button.bind('click', month, changeWeather);
    }
...
function makeRain() {
    ...
    var lineLength = 14; // currentWeather.type == "thunder" ? 35 : 14;
    ...
    TweenMax.fromTo(
        ...
        { x: x, y: 0 - lineLength },
        {
            ...
            onCompleteParams: [line, lineWidth, x, currentMonth.name],
        },
    );
}
...
function tick() {
    ...
    for (var i = 0; i < clouds.length; i++) {
        if (currentMonth.name == 'july' || currentMonth.name == 'june' || currentMonth.name == 'august') {
...
function reset() {
    ...
    for (var i = 0; i < months.length; i++) {
        container.removeClass(months[i].name);
        months[i].button.removeClass('active');
    }
}
...
function declineWord(number, word1, word2, word5) {
...
function updateTimer() {
    ...
    const secondsDiv = $('#seconds');
    const daysTextDiv = $('#days-text');
    const hoursTextDiv = $('#hours-text');
    const minutesTextDiv = $('#minutes-text');
    const secondsTextDiv = $('#seconds-text');
    
    daysDiv.text(days.toString());
    hoursDiv.text(fd(hours));
    minutesDiv.text(fd(minutes));
    secondsDiv.text(fd(seconds));

    daysTextDiv.text(declineWord(days, 'день', 'дня', 'дней'));
    hoursTextDiv.text(declineWord(hours, 'час', 'часа', 'часов'));
    minutesTextDiv.text(declineWord(minutes, 'минута', 'минуты', 'минут'));
    secondsTextDiv.text(declineWord(seconds, 'секунда', 'секунды', 'секунд'));
}

function updateSummaryText() {
    summary.html(currentMonth.label);
    TweenMax.fromTo(
        summary,
        1.5,
        { x: 30 },
...
function changeWeather(month) {
    if (month.data) month = month.data;
    reset();
